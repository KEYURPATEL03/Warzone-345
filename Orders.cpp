...
ostream& operator<<(ostream& os, Orders& ord){
    os << "This is an/a "<< ord.getName() <<" object." << endl;
    if(ord.getEffect() != ""){
...
//Stream Insertion Operator Definition for the Deploy subclass
...
//Stream Insertion Operator Definition for the Advance subclass
...
//Stream Insertion Operator Definition for the Bomb subclass
...
//Stream Insertion Operator Definition for the Blockade subclass
...
//Stream Insertion Operator Definition for the Airlift subclass
...
//Stream Insertion Operator Definition for the Negotiate subclass
...
//The getListOfOrders() method for the OrdersList class:
vector<Orders*> OrdersList::getListOfOrders(){
    return this->listOfOrders;
}

//The remove method for the OrdersList class:
void OrdersList::remove(int removingIndex){
    //We are doing removingIndex-1 to make it easier to know which order we are removing. This way 1 corresponds to the first order instead of 0
    removingIndex = removingIndex - 1;
    //The if function is assuring that the numbers given for the indices are not out of bounds
    if(removingIndex >= 0 && removingIndex < listOfOrders.size()){
        listOfOrders.erase(listOfOrders.begin() + removingIndex);
    }
...
void OrdersList::move(int startIndex, int endIndex){
    //The if function is assuring that the numbers given for the indices are not out of bounds
    if(startIndex >= 0 && startIndex < listOfOrders.size() && endIndex >= 0 && endIndex < listOfOrders.size() ){
        Orders* tempPlaceholder;
        tempPlaceholder = this->listOfOrders[startIndex];
        this->listOfOrders.erase(listOfOrders.begin() + startIndex);
        this->listOfOrders.insert(listOfOrders.begin() + endIndex , tempPlaceholder);
    }
